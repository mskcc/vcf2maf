...
86. CLIN_SIG - clinical significance of variant from dbSNP (http://www.ncbi.nlm.nih.gov/clinvar/docs/clinsig/) Grep for drug_response and pathogenic
...
72. SIFT - the SIFT prediction and/or score, with both given as prediction (score)
73. PolyPhen - the PolyPhen prediction and/or score
109. FILTER - False-positive filtering status, borrowed from the input MAF/VCF. A tag named "common_variant" is also added here, if minor allele freq is >0.0004 in any ExAC subpopulation, unless ClinVar says pathogenic, risk_factor, or protective. This means the variant is likely a false-positive somatic call at a germline position.
