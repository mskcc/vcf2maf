...
ok( system( "docker run --rm -v $test_dir:/opt/tests vcf2maf:main perl vcf2maf.pl --vep-path /usr/local/bin --vep-data tests --vcf-tumor-id TUMOR --vcf-normal-id NORMAL --tumor-id MSK_T001 --normal-id MSK_N001 --maf-center mskcc.org --vep-forks 1 --buffer-size 50 --ncbi-build GRCh38 --input-vcf tests/test_b38.vcf --output-maf tests/test_b38_output.more.new.maf --vep-custom tests/test_b38.gnomad.exomes.r2.1.1.sites.vcf.gz,gnomAD,vcf,exact,,AC --retain-ann gnomAD_AC --ref-fasta tests/Homo_sapiens.GRCh38.dna.chromosome.21.fa --retain-fmt GT" ) == 0 );
