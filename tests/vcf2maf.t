...
ok( system( "docker run --rm vcf2maf:master perl vcf2maf.pl --help > /dev/null" ) == 0 );
ok( system( "docker run --rm vcf2maf:master perl vcf2maf.pl --man > /dev/null" ) == 0 );

# Test standard operation, diff, and cleanup
ok( system( "docker run --rm -v $test_dir:/opt/tests vcf2maf:master perl vcf2maf.pl --vep-path /usr/local/bin --online --ncbi-build GRCh38 --input-vcf tests/test_b38.vcf --output-maf tests/test_b38_output.online.new.maf --ref-fasta tests/Homo_sapiens.GRCh38.dna.chromosome.13.fa" ) == 0 );
