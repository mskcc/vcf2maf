...
my $retain_cols = "Entrez_Gene_Id,Center,Verification_Status,Validation_Status,Mutation_Status,Sequencing_Phase,Sequence_Source,Validation_Method,Score,BAM_file,Sequencer,Tumor_Sample_UUID,Matched_Norm_Sample_UUID";
...
( -s $maf2vcf_path ) or die "ERROR: Couldn't locate maf2vcf.pl script! Must be in the same folder as maf2maf.pl\n";
( -s $vcf2maf_path ) or die "ERROR: Couldn't locate vcf2maf.pl script! Must be in the same folder as maf2maf.pl\n";
...
if( $retain_cols ) {
    ...
    while( my $line = $input_maf_fh->getline ) {
        ...
        if( $line =~ m/^(Hugo_Symbol|Chromosome)/ ) {
            ...
            # Create a unique key for this variant using Chromosome:Start_Position:Tumor_Sample_Barcode
            ...
            foreach my $c ( split( ",", $retain_cols )) {
                $c = lc( $c );
                $input_maf_data{$key}{$c} = $cols[$input_maf_col_idx{$c}] if( defined $input_maf_col_idx{$c} and defined $cols[$input_maf_col_idx{$c}] );
            }
        }
    }
    $input_maf_fh->close;
    ...
    foreach my $tn_maf ( @mafs ) {
        ...
        my $tmp_tn_maf_fh = IO::File->new( "$tn_maf.tmp", ">" ) or die "ERROR: Couldn't open output file: $tn_maf.tmp\n";
...
=head1 OPTIONS
 ...
 --retain-cols    Comma-delimited list of columns to retain from the input MAF [Entrez_Gene_Id,Center,Verification_Status,Validation_Status,Mutation_Status,Sequencing_Phase,Sequence_Source,Validation_Method,Score,BAM_file,Sequencer,Tumor_Sample_UUID,Matched_Norm_Sample_UUID]
